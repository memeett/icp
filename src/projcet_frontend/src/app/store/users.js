...
export const freelancersAtom = atom([]);
...
export const filteredFreelancersAtom = atom((get) => {
    ...
    if (filters.skills.length > 0) {
        filtered = filtered.filter(user => user.preference.some(pref => filters.skills.includes(pref.jobCategoryName)));
    }
    // Apply rating filter
    if (filters.rating > 0) {
...
// User actions atom
export const userActionsAtom = atom(null, (get, set, action) => {
    switch (action.type) {
        case 'SET_USERS':
        ...
        case 'ADD_USER':
            ...
            break;
        case 'UPDATE_USER':
            set(usersAtom, (prev) => prev.map(user => user.id === action.userId ? { ...user, ...action.updates } : user));
            break;
        case 'DELETE_USER':
...
export const favoriteFreelancersAtom = atom([]);
export const isFavoriteFreelancerAtom = atom((get) => (userId) => get(favoriteFreelancersAtom).includes(userId), (get, set, { userId, favorite }) => {
