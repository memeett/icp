...
export const createJobTransaction = async (ownerId, jobId) => {
...
//     }
// }
export const getActiveTransactionByFreelancer = async (freelancerId) => {
    const authClient = await AuthClient.create();
    const identity = authClient.getIdentity();
    const agent = new HttpAgent({ identity });
    if (process.env.DFX_NETWORK === "local") {
    ...
    try {
        ...
        const jobTransactions = await Promise.all(res.map(async (jt) => {
            try {
        ...
        const filteredTransactions = jobTransactions.filter((jt) => jt !== null);
...
export const getFreelancerHistory = async (freelancerId) => {
    const authClient = await AuthClient.create();
    const identity = authClient.getIdentity();
    const agent = new HttpAgent({ identity });
    if (process.env.DFX_NETWORK === "local") {
