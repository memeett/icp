...
export const createInbox = async (receiverId, jobId, senderId, inbox_type, message) => {
    const agent = await agentService.getAgent();
    try {
        const result = await inbox.createInbox(receiverId, jobId, senderId, inbox_type);
        if ("ok" in result) {
            return result.ok;
        }
...
export const getReceiverInbox = async (receiverId) => {
    const agent = await agentService.getAgent();
    try {
...
export const getSenderInbox = async (senderId) => {
    const agent = await agentService.getAgent();
    try {
...
//   }
// };
export const getAllInboxBySubmissionType = async (submissionType) => {
    const agent = await agentService.getAgent();
    try {
