...
export const createJob = async (jobName, jobDescription, jobTags, jobSalary, jobSlots) => {
    const agent = await agentService.getAgent();
    try {
        if (!jobName.trim())
            return ["Failed", "Job name is required"];
        ...
        if (jobSlots < 1)
            return ["Failed", "Job slots must be at least 1"];
...
export const updateJob = async (jobId, jobName, jobDescription, jobTags, jobSalary, jobSlots, jobStatus) => {
    const agent = await agentService.getAgent();
    try {
        // Validation checks
        if (!jobName.trim())
        ...
        if (jobSlots < 1)
            return ["Failed", "Job slots must be at least 1"];
        if (!jobStatus)
            return ["Failed", "Job status is required"];
        const userData = localStorage.getItem("current_user");
        const newJobTags = [];
        // Process job tags
        for (const tag of jobTags) {
...
export const viewAllJobs = async () => {
    const agent = await agentService.getAgent();
    try {
...
export const viewAllJobCategories = async () => {
    const agent = await agentService.getAgent();
    try {
...
export const getJobApplier = async (jobId) => {
    const agent = await agentService.getAgent();
    try {
        ...
        const processedUsers = await Promise.all(appliers.map(async (applierData) => {
            ...
            return {
                user: {
                    ...
                    preference: userData.preference.map((pref) => ({
                        ...pref,
                        id: pref.id.toString()
                    }))
                },
                appliedAt: BigInt(applierData.appliedAt), // Ensure this field is included
            }; // Assert that this matches the ApplierPayload type
        }));
...
export const getAcceptedFreelancer = async (jobId) => {
    const agent = await agentService.getAgent();
...
export const startJob = async (user_id, job_id, amount) => {
    try {
        ...
        if ("ok" in result) {
            ...
            if (userStr) {
                ...
                if (parsedData && typeof parsedData === "object" && "ok" in parsedData) {
                    ...
                    localStorage.setItem("current_user", JSON.stringify({ ok: updatedUser }));
                    return {
                        jobStarted: true,
...
export const getUserJobByStatusFinished = async (userId) => {
    const agent = await agentService.getAgent();
    try {
...
export const finishJob = async (job_id) => {
    try {
