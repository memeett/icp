...
export const getProfilePictureUrl = (userId, blob) => {
...
export const login = async (principalId) => {
    ...
    try {
        ...
        if ("ok" in userIdResult) {
            ...
            if ("ok" in userDetail) {
                ...
                document.cookie = `cookie=${encodeURIComponent(JSON.stringify(res))}; path=/; Secure; SameSite=Strict`;
                const needsCompletion = !(userData.isProfileCompleted || false);
...
let userCache = { user: null, timestamp: 0 };
...
export const updateUserProfile = async (payload) => {
    try {
        ...
        if (payload.username !== undefined)
            backendPayload.username = [payload.username];
        ...
        if (payload.isFaceRecognitionOn !== undefined)
            backendPayload.isFaceRecognitionOn = [payload.isFaceRecognitionOn];
        if (payload.isProfileCompleted !== undefined)
            backendPayload.isProfileCompleted = [payload.isProfileCompleted];
