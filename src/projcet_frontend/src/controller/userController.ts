...
import { resetJobCategoriesCache } from "../utils/jobCategoriesCache";
...
export const validateCookie = async (): Promise<boolean> => {
  ...
  try {
    ...
    if (!isValid) {
      document.cookie =
        "cookie=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; Secure; SameSite=Strict";
...
export const updateUserProfile = async (
  ...
  try {
    ...
    // Process category preferences if provided
    if (payload.preference) {
      for (const categories of payload.preference) {
        for (const category of categories) {
          let existingCategory = await job.findJobCategoryByName(
            category.jobCategoryName
          );

          if (!("ok" in existingCategory)) {
            existingCategory = await job.createJobCategory(
              category.jobCategoryName
            );
          }

          if ("ok" in existingCategory) {
            newCategories.push(existingCategory.ok);
          }
        }
      }
      ...
      // Invalidate job categories cache
...
export const getAllUsers = async (): Promise<User[] | null> => {
  try {
    ...
    console.log(
      "Users with profile pictures (excluding current user):",
