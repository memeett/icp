...
// please help me create get user by their id
export const getUserById = async (userId: string): Promise<User | null> => {
  try {
    const userRes = await user.getUserById(userId);

    if ("ok" in userRes) {
      //   console.log("User found:", userRes.ok);
      const userParsed = userRes.ok;
      console.log("User parsed:", userParsed);
      const convertedUser: User = {
        ...userParsed,
        profilePicture: userParsed.profilePicture
          ? new Blob([new Uint8Array(userParsed.profilePicture)], { type: "image/jpeg" })
          : new Blob([], { type: "image/jpeg" }),
      };
      return convertedUser; // Return the user object
    } else {
      console.error("Error fetching user:", userRes.err);
